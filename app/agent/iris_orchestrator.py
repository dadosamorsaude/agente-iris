...
async def run_iris_agent(
    ...
    # da memoria de treinamento (ex.: "2025").
    hoje_dt = date.today()
    hoje_br = hoje_dt.strftime("%d/%m/%Y")
    data_ontem = (hoje_dt - timedelta(days=1)).strftime("%d/%m/%Y")
    ...
    TOOL_ALIASES = {
        ...
        "fetch_curated_lessons": "Consulta de Lições Curadas (Memória)",
    }
    ...
    try:
        ...
        logger.exception("Erro durante a execução do stream do agente Iris")
    ...
    sql_used = len(raw_athena) > 0
    rag_used = len(raw_rag) > 0
